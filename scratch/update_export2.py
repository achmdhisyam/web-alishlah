...
# Find the second thead and replace it
content = re.sub(r'<table class="tabelku table-sm" id="example1">\s*<thead>.*?</thead>', '<table class="tabelku table-sm" id="example1">\n' + new_thead, content, flags=re.DOTALL)
# Replace the second tbody
content = re.sub(r'<tbody>\s*<\?php \$no=1; foreach\(\$siswa as \$siswa\) \{ \?>.*?</tbody>', new_tbody, content, flags=re.DOTALL)

with open(file_path, "w", encoding="utf-8") as f:
    f.write(content)

print("export.php updated again.")
