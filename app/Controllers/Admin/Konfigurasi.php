...
class Konfigurasi extends BaseController
{
	...
	public function pendaftaran()
	{
		...
		if($this->request->getMethod() === 'POST' && $this->validate(
			...
			$data = [	'title'			=> 'Buka atau Tutup Formulir SPMB Online',
						'konfigurasi'	=> $konfigurasi,
	...
	// pembayaran (Rincian Administrasi)
	public function pembayaran()
	{
		...
		if($this->request->getMethod() === 'POST' && $this->validate(
			[
            'rincian_administrasi' 	=> 'required',
        	])) {
			// masuk database
			$data = [	'id_konfigurasi'		=> $konfigurasi->id_konfigurasi,
						'id_user'				=> $this->session->get('id_user'),
						'rincian_administrasi'	=> $this->request->getPost('rincian_administrasi'),
						'syarat_pendaftaran'	=> $this->request->getPost('syarat_pendaftaran')
					];
			$m_konfigurasi->edit($data);
			// masuk database
			$this->session->setFlashdata('sukses','Data rincian administrasi telah diupdate');
			return redirect()->to(base_url('admin/konfigurasi/pembayaran'));
	    }else{
			$data = [	'title'			=> 'Rincian Administrasi Pembayaran',
						'konfigurasi'	=> $konfigurasi,
