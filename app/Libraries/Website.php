...
class Website
{
	...
	{
		if(strlen($url) <= 11) return $url; // Sudah berupa ID
	...
	// youtube_thumbnail
	public function youtube_thumbnail($url, $gambar_manual = '')
	{
		if ($gambar_manual != "" && $gambar_manual != NULL) {
			return base_url('assets/upload/image/' . $gambar_manual);
		} else {
			$video_id = $this->youtube_id($url);
			if($video_id != "") {
				return "https://img.youtube.com/vi/" . $video_id . "/mqdefault.jpg";
			} else {
				return base_url('assets/upload/image/no-image.png'); // Fallback
			}
		}
	}
