...
<form action="<?php echo base_url('admin/profil/edit/'.$profil->id_profil) ?>" method="post" accept-charset="utf-8">
<?php 
echo csrf_field(); 
?>

<input type="hidden" name="tanggal_publish" value="<?php echo date('d-m-Y', strtotime($profil->tanggal_publish)) ?>">
<input type="hidden" name="jam" value="<?php echo date('H:i:s', strtotime($profil->tanggal_publish)) ?>">
<input type="hidden" name="ringkasan" value="<?php echo $profil->ringkasan ?>">

<div class="form-group row">
	<label class="col-md-2">Judul Halaman <span class="text-danger">*</span></label>
	<div class="col-md-10">
		<input type="text" name="judul_profil" class="form-control" placeholder="Contoh: Sejarah Singkat Sekolah" value="<?php echo $profil->judul_profil ?>" required>
	</div>
</div>
...
<div class="form-group row">
	...
	<div class="col-md-10">
		...
		<textarea id="isi" name="isi" class="form-control konten" rows="12" required><?php echo $profil->isi ?></textarea>
	</div>
</div>
...
<div class="form-group row">
	...
	<div class="col-md-3">
		<select name="status_profil" class="form-control">
			...
			<option value="Draft" <?php if($profil->status_profil=="Draft") { echo 'selected'; } ?>>Draft</option>
		</select>
	</div>
</div>
